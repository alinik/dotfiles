...
complete -c codex -n "__fish_codex_using_subcommand help; and not __fish_seen_subcommand_from exec review login logout mcp plugin mcp-server app-server remote-control app completion update doctor sandbox debug execpolicy apply resume archive delete unarchive fork cloud responses-api-proxy stdio-to-uds exec-server features help" -f -a "app" -d 'Launch the Codex desktop app (opens the app installer if missing)'
